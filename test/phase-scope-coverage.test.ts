...
describe('PHASE_SCOPE coverage', () => {
  ...
  test('all 19 phases covered (regression on accidental omission)', () => {
    ...
    // without updating PHASE_SCOPE notices here too. v0.39.1.0 brought
    // `schema-suggest` (17th); the v0.40 link-wave added `infer_links`
    // (which had been missing from PHASE_SCOPE — repaired in the PC3 PR)
    // and `link_mine` (PC3 cosine edge mining) → 19.
